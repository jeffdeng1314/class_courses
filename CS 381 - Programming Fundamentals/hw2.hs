...
pathTo a (Node x l r) = if (a == x) then (Just []) else (adding (makeStep L (pathTo a l) ) (makeStep R (pathTo a r)))

adding :: Maybe Path -> Maybe Path -> Maybe Path
adding Nothing Nothing = Nothing
adding (Just a) Nothing = Just a
adding Nothing (Just b) = Just b
adding (Just a) (Just b) = Just (a ++ b)
  
makeStep :: Step -> Maybe Path -> Maybe Path
makeStep a (Just b) = Just ([a] ++ b) 
makeStep a Nothing = Nothing
