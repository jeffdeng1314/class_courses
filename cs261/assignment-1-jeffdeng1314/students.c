...
void init_student(struct student* student, char* name, int id, float gpa) {
	...
	student->name = malloc((strlen(name+1))*sizeof(char));//create dynamic memory for name 
	strcpy(student->name, name);//use strcpy to copy over the char array because you can't just assign name into student->name
	student->id = id;
	student->gpa = gpa;
	//printf("Inside init_student\n");
...
//	 printf("Inside the free_student\n");
	 free(student->name);
	 //free(student);
...
struct student* deep_copy_student(struct student* student) {
	...
	student_copy->name = malloc((strlen(student->name)+1)*sizeof(char));//allocate dynamic memories for a copy of students
...
struct student* create_student_array(int num_students, char** names, int* ids,
    float* gpas) {
		...
		struct student *array_of_students = malloc(num_students*sizeof(struct student));//struct student is a type (the whole thing, not just student)

		int i;

		for(i = 0; i < num_students; i++){

			// array_of_students[i].id = ids[i];
			// array_of_students[i].gpa = gpas[i];
			// array_of_students[i].name = malloc((strlen(names[i])+1)*sizeof(char));
			// strcpy(array_of_students[i].name,names[i]);

			init_student(&array_of_students[i],names[i], ids[i],gpas[i]);
...
//	struct student *high_gpa_student = malloc(sizeof(struct student));
	int i, keeping_track;
	float highest_gpa = 0;
	for (i = 0; i < num_students; i++){
	...
	// high_gpa_student->name = malloc((strlen(students[keeping_track].name)+1)*sizeof(char));
	// strcpy(high_gpa_student->name, students[keeping_track].name);

	// high_gpa_student->gpa = students[keeping_track].gpa;
	// high_gpa_student->id = students[keeping_track].id;

	// printf("This student is: %s\n", students[keeping_track].name );
	// printf("This is the highest gpa: %f\n", students[keeping_track].gpa );


  //return high_gpa_student;
	return &students[keeping_track];
}
...
struct student* find_min_gpa(struct student* students, int num_students) {
	...
	//struct student *low_gpa_student = malloc(sizeof(struct student));
	int i, keeping_track;
	float low_gpa = students[0].gpa;
	for (i = 0; i < num_students; i++){
		if(students[i].gpa < low_gpa){
			low_gpa = students[i].gpa;
			keeping_track = i;
		}

	}

	// low_gpa_student->name = malloc((strlen(students[keeping_track].name)+1)*sizeof(char));
	// strcpy(low_gpa_student->name, students[keeping_track].name);

	// low_gpa_student->gpa = students[keeping_track].gpa;
	// low_gpa_student->id = students[keeping_track].id;
...
void sort_by_gpa(struct student* students, int num_students) {
	...
	struct student temp;//temp container
