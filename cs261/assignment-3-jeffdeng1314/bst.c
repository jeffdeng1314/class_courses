...
int recursion(struct bst_node* node){
...
int recursion_sum(struct bst_node* node, int sum){
 // assert(node);
  ...
  if (node == NULL){
    return 0;
      ...
      if(recursion_sum(node->left,sum-(node->val))){
        return 1;
      }
      if(recursion_sum(node->right,sum-(node->val))){
        return 1;
      }
...
void push_to_stack(struct stack* s, struct bst_node* node){
  while(node){
    stack_push(s,node);
    node = node->left;
  }
...
// int bst_iterator_isempty(struct bst_iterator* iter){
//   assert(iter);
...
int bst_iterator_has_next(struct bst_iterator* iter) {
 // printf("inside has neasdfasdfaxt\n");
...
int bst_iterator_next(struct bst_iterator* iter) {
  ...
  struct bst_node* temp = stack_pop(iter->s);
 // printf("cur: %d\n",iter->cur->val);



  push_to_stack(iter->s,temp->right);
