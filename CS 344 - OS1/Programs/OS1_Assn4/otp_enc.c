...
int file_reading(char *argv, int type){
	...
	while(!feof(f_pointer)){
		...
		if(isalpha(character) || character == ' '){
			...
			fprintf(stderr,"opt_enc error: input contains bad characters\n");
			exit(1);
		}
...
int main(int argc, char *argv[])
{
	...
	strcpy(concat,"e");//file type indicator
	...
	// Send message to server
		...
		while(1){
			if(charWrite == strlen(concat))
				break;
			charWrite = send(socketFD, concat, strlen(concat), 0);
		}
		if (charWrite < 0) error("CLIENT: ERROR writing to socket");

	
	// if (charWrite < strlen(concat)) printf("CLIENT: WARNING: Not all data written to socket!\n");
	...
	int x;
		...
		while(strstr(key,"#") == NULL){
			if(strcmp(key,"Invalid") == 0){ //gets the message from the server if it's invalid
				fprintf(stderr, "otp_enc cannot access otp_enc_d\n");
				exit(2); //exit
			}
			char buffer[200000];
			memset(buffer, '\0', sizeof(buffer));
			charsRead = recv(socketFD, buffer, sizeof(buffer), 0); //keeps receiving info from the server
